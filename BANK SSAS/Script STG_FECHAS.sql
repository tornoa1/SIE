...
/********Script de carga para la tabla STG_FECHAS********

Objetivo : Poblar la tabla STG_FECHAS de la base STAGE
Metodo   : Empleando un EXECUTE SQL TASK, copiar todo este script en la propiedad de SQLCOMMAND DEL PACKAGE DE SSIS
Tarea    : Completar la sintaxis del script siguiente para obtener las variables respectivas y realizar el INSERT
...
BEGIN TRANSACTION
    ...
    --Rango de fechas a generar: del 01/01/2017 al 31/12/2022
    SELECT @FechaDesde = CAST('20170101' AS date)
	SELECT @FechaHasta = CAST('20221231' AS date)
